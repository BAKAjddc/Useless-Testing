...
local TrulyNotFolder = "TrulyNot"
local ConfigurationFolder = TrulyNotFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		rayfieldOpen = {Type = 'bind', Value = 'K', Name = 'TrulyNot Keybind'},
		-- buildwarnings
		-- rayfieldprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(TrulyNotFolder) then
				if isfile and isfile(TrulyNotFolder..'/settings'..ConfigurationExtension) then
					file = readfile(TrulyNotFolder..'/settings'..ConfigurationExtension)
				end
			end
	...
	if not success then 
		if writefile then
			warn('TrulyNot had an issue accessing configuration saving capability.')
		end
	end
end
...
repeat
	if TrulyNot:FindFirstChild('Build') and TrulyNot.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('TrulyNot | Build Mismatch')
		print('TrulyNot may encounter issues as you are running an incompatible interface version ('.. ((TrulyNot:FindFirstChild('Build') and TrulyNot.Build.Value) or 'No Build') ..').\n\nThis version of TrulyNot is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end
...
elseif syn and syn.protect_gui then 
	syn.protect_gui(TrulyNot)
	TrulyNot.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	TrulyNot.Parent = CoreGui:FindFirstChild("RobloxGui")
...
local Main = TrulyNot.Main
local MPrompt = TrulyNot:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = TrulyNot:FindFirstChild('Drag')
...
TrulyNot.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/TrulyNot/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = TrulyNot.Notifications

local SelectedTheme = TrulyNotLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = TrulyNotLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	TrulyNot.Main.BackgroundColor3 = SelectedTheme.Background
	TrulyNot.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	TrulyNot.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	TrulyNot.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	TrulyNot.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	TrulyNot.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	TrulyNot.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		TrulyNot.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		TrulyNot.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(TrulyNot:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("TrulyNot | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("TrulyNot | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('TrulyNot had an issue decoding the configuration file, please try delete the file and reopen TrulyNot.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(TrulyNotLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("TrulyNot | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--TrulyNotLibrary:Notify({Title = "TrulyNot Flags", Content = "TrulyNot was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(TrulyNotLibrary.Flags) do
...
function TrulyNotLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			TrulyNotLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show TrulyNot'.", Duration = 7, Image = 4400697855})
		else
			TrulyNotLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {settingsTable.General.rayfieldOpen.Value or 'K'}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(TrulyNotFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('TrulyNot Settings', 0, true)

	if TabList['TrulyNot Settings'] then
		TabList['TrulyNot Settings'].LayoutOrder = 1000
	end

	if Elements['TrulyNot Settings'] then
		Elements['TrulyNot Settings'].LayoutOrder = 1000
	end
...
function TrulyNotLibrary:CreateWindow(Settings)
	...
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				TrulyNotLibrary:Notify({Title = 'Build Mismatch', Content = 'TrulyNot may encounter issues as you are running an incompatible interface version ('.. ((TrulyNot:FindFirstChild('Build') and TrulyNot.Build.Value) or 'No Build') ..').\n\nThis version of TrulyNot is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end

	if isfolder and not isfolder(TrulyNotFolder) then
		makefolder(TrulyNotFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "TrulyNot"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "TrulyNot Interface Suite" then
		LoadingFrame.Version.Text = "TrulyNot UI"
	end
	...
	if not Settings.DisableTrulyNotPrompts then
		task.spawn(function()
			while true do
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(TrulyNotFolder.."/Discord Invites") then
			makefolder(TrulyNotFolder.."/Discord Invites")
		end

		if isfile and not isfile(TrulyNotFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(TrulyNotFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"TrulyNot RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(TrulyNotFolder.."/Key System") then
			makefolder(TrulyNotFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("TrulyNot | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with TrulyNot specific development.')
				end
			end
		end
		...
		if isfile and isfile(TrulyNotFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(TrulyNotFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(TrulyNotFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						TrulyNotLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("TrulyNot | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with TrulyNot specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= TrulyNotLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					TrulyNotLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			TrulyNot.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("TrulyNot | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with TrulyNot specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					TrulyNotLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("TrulyNot | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with TrulyNot specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					TrulyNotLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("TrulyNot | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with TrulyNot specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					TrulyNotLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= TrulyNotLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("TrulyNot | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with TrulyNot specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						TrulyNotLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("TrulyNot | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with TrulyNot specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					TrulyNotLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			TrulyNotLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			TrulyNotLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('TrulyNot had an issue creating settings.') end
...
function TrulyNotLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function TrulyNotLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function TrulyNotLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['TrulyNot Settings'])
		end)
	end)
...
function TrulyNotLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				TrulyNotLibrary:Notify({Title = "TrulyNot Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			TrulyNotLibrary:Notify({Title = "TrulyNot Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('TrulyNot Configurations Error | '..tostring(result))
			TrulyNotLibrary:Notify({Title = "TrulyNot Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	local Window = TrulyNotLibrary:CreateWindow({
		Name = "TrulyNot Example Window",
		LoadingTitle = "TrulyNot Interface Suite",
		Theme = 'Default',
		Icon = 0,
		...
		KeySettings = {
			...
			FileName = "Key", -- It is recommended to use something unique as other scripts using TrulyNot may overwrite your key file
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like TrulyNot to get the key from
	...
	--TrulyNotLibrary:Notify({Title = "TrulyNot Interface", Content = "Welcome to TrulyNot. These - are the brand new notification design for TrulyNot, with custom sizing and TrulyNot calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(TrulyNotLibrary.Theme) do
...
task.delay(4, function()
	TrulyNotLibrary.LoadConfiguration()
